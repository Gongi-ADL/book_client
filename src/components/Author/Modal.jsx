...
import { handleDelete, handleDeleteAuthor } from '../../utils/api/fetch/axiosActions'
import {toast} from 'react-toastify'
import { useNavigate } from 'react-router-dom'

const Modal = ({isOpen, authorid}) => {
  const Navigate = useNavigate()
  const notifybyDelete = () => {
    toast.success('Author was succesfully deleted!', {
      position: "top-right",
      autoClose: 2000,
  ...
  const deleteAuthor = async (event) => {
        try {
          ...
          await handleDeleteAuthor(authorid)
    ...
    <div className='bg-gray-400 bg-opacity-50 fixed left-0 bottom-0 h-screen w-screen flex justify-center items-center'>
        <div className='bg-white container w-[400px] h-[250px] flex flex-col gap-1 p-5 rounded-md shadow-md'>
            <div className='text-center w-full h-[200px]'>
                <h1 className='text-black font-extrabold font-[Poppins] text-4xl'>Want to delete this author?</h1>
            ...
            <div className='flex flex-wrap gap-3 items-center justify-center'>
                ...
                <a className='p-2 w-28 text-center text bg-red-400 text-black hover:bg-red-800 hover:text-white hover:font-bold cursor-pointer' onClick={deleteAuthor}> Yes </a>
            </div>
