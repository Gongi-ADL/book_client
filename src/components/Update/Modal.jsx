...
const Modal = ({isOpen, bookId, isData}) => {
    const Navigate = useNavigate()
  const notifyByUpdate = () => {
    toast.success('Book was succesfully updated!', {
      position: "top-right",
      autoClose: 2000,
  ...
  const updateBook = async (event) => {
        try {
          ...
          await handleUpdate(bookId, isData)
          notifyByUpdate()
        } catch (error) {
          console.error(error)
        }
    ...
    <div className='bg-gray-400 bg-opacity-50 fixed left-0 bottom-0 h-screen w-screen flex justify-center items-center'>
        <div className='bg-white container w-[400px] h-[250px] flex flex-col gap-1 p-5 rounded-md shadow-md'>
            <div className='text-center w-full h-[200px]'>
                <h1 className='text-black font-extrabold font-[Poppins] text-4xl'>Want to update this book?</h1>
            ...
            <div className='flex flex-wrap gap-3 items-center justify-center'>
                ...
                <a className='p-2 w-28 text-center text bg-red-400 text-black hover:bg-red-800 hover:text-white hover:font-bold cursor-pointer' onClick={updateBook}> Yes </a>
            </div>
